...
const UserCard = ({ user, onLike, onDislike, disabled = false }) => {
  ...
  return (
    <article className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-7 bg-slate-900/60 border border-slate-800 rounded-3xl w-full shadow-2xl hover:border-rose-500/30 hover:shadow-rose-500/5 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
      ...
      <div className="flex flex-row gap-3 items-center shrink-0 z-10">
        <button
          className="px-6 py-3.5 text-xs font-black uppercase tracking-wider rounded-full bg-white/5 text-slate-300 border border-white/10 hover:bg-rose-500/10 hover:border-rose-500/30 hover:text-rose-400 active:scale-95 transition-all duration-200 disabled:opacity-50 disabled:pointer-events-none"
          onClick={() => {
            playSound("sad", 0.4);
        ...
        <button
          className="px-6 py-3.5 text-xs font-black uppercase tracking-wider rounded-full bg-gradient-to-r from-rose-500 to-pink-600 text-white shadow-lg shadow-rose-500/20 hover:from-rose-600 hover:to-pink-700 active:scale-95 transition-all duration-200 disabled:opacity-50 disabled:pointer-events-none"
          onClick={() => {
            playSound("vine", 0.5);
